...
//
// hydra_monitor.cpp : Interacts with driver_hydra to provide overlay instructions at startup
//

#include "stdafx.h"
#include "hydra_monitor.h"
#include <openvr.h>
...
{
public:
	CHydraMonitor( const std::string & path )
...
protected:
	...
	void MainLoop()
	{
		while ( true )
		{
			...
			ShowOverlay( BestOverlayForHydraDevices() );
			...
			while ( vr::VRSystem()->PollNextEvent( &Event, sizeof( Event ) ) )
			{
				...
				case vr::VREvent_VendorSpecific_Reserved_Start + 0:
					...
					if ( IsHydraDevice( Event.trackedDeviceIndex ) )
					{
	...
	bool ShowOverlay( EOverlayToDisplay eOverlay )
	{
		...
		vr::EVROverlayError eOverlayError = vr::VROverlay()->CreateOverlay( "hydra_monitor", "Hydra Monitor", &m_OverlayHandle );
	...
	void UpdateTrackedDevice( uint32_t unTrackedDeviceIndex )
	{
		...
		{
			if ( strcmp( rgchTrackingSystemName, "hydra" ) == 0 )
			{
				m_setHydraDevices.insert( unTrackedDeviceIndex );
			}
		}
	}

	/** If any Hydra devices need automatic hemisphere tracking enabled, prompt the user to do that.
	 * Otherwise, if any devices do not know how to transform into the global coordinate system, show
	 * instructions for that. */
	EOverlayToDisplay BestOverlayForHydraDevices()
	{
		...
		for ( auto it = m_setHydraDevices.begin(); it != m_setHydraDevices.end(); ++it )
		{
	...
	bool IsHydraDevice( uint32_t unTrackedDeviceIndex )
	{
		return ( m_setHydraDevices.count( unTrackedDeviceIndex ) != 0 );
	}
...
private:
	...
	std::set<uint32_t> m_setHydraDevices;
};
                     ...
                     _In_ int       nCmdShow)
{
	...
	CHydraMonitor HydraMonitor( resources );
